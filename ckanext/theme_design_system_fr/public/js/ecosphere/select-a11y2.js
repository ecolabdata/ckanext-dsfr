...
class Select {
  constructor(el, options) {
    //console.log('creating a11y select:', el)
    this.el = el;
  ...
  _createButton() {
    ...
    if (this.multiple) {
      ...
      //console.log('selected index: ', this.el.selectedIndex);
  ...
  _fillSuggestions() {
    ...
    this.suggestions = Array.prototype.map.call(this.el.options, function (option, index) {
      ...
      for(var i = 0; i < option.classList.length; i++)
        suggestion.classList.add(option.classList[i]);
      ...
      if(option.hasAttribute('data-href')) {
        ...
        sugLink.setAttribute('href', option.getAttribute('data-href'));
        //sugLink.setAttribute('href', "#");
        sugLink.innerText = option.label || option.value;
        suggestion.appendChild(sugLink);
      }
      else
  ...
  _toggleSelection(optionIndex, close = true) {
    ...
    // Ajout ci-dessous pour pouvoir écouter les changements sur l'élément
